...
const ActionLogMessage:FC<MessageProps> = ({message}) => {
  return (
    <div className="bg-neutral text-stone-800 ActionLog-message flex flex-col">
      <div className="flex flex-row p-2">
        <p className="bg-warning m-1">{message.name}</p>
        <p className="bg-success m-1 "> {message.action} {message.action === 'ATTACKED' ? <i className ="fa-solid fa-gavel"></i> : message.action === 'CASTED' ? <i className ="fa-solid fa-wand-sparkles"></i> : message.action === 'TALKED' ? <i className ="fa-solid fa-head-side-cough"></i> : message.action === 'GAVE' ? <i className ="fa-solid fa-hand-holding-dollar"></i> : message.action === 'ROLLED' ? <i className ="fa-solid fa-dice-d20"></i> : null}</p>
        {message.actionName ? <p className="bg-error m-1 "> {message.action === 'ATTACKED' ? `using "${message.actionName}"` : message.actionName} </p> : null}
        <p className="bg-info m-1 ">{message.target ? '@' : null} {message.target}</p>
        <p className=" m-1 ">{message.dialog}</p>
      </div>
