...
const { data: reactionRows, response: reactionResponse } = await request('/rest/v1/rpc/create_reaction_prompt', { token: outsider.token, method: 'POST', body: { requested_session_id: sessionId, requested_campaign_id: campaignId, requested_character_id: characterId, requested_target_user_id: owner.id, requested_prompt: 'Use Shield against the incoming attack?', duration_seconds: 5 } })
assert.equal(reactionResponse.status, 200)
assert.equal(reactionRows.created_by, outsider.id)
assert.ok(Date.parse(reactionRows.expires_at) > Date.parse(reactionRows.created_at))
const { response: invalidReactionDurationResponse } = await request('/rest/v1/rpc/create_reaction_prompt', { token: outsider.token, method: 'POST', body: { requested_session_id: sessionId, requested_campaign_id: campaignId, requested_character_id: characterId, requested_target_user_id: owner.id, requested_prompt: 'Invalid duration', duration_seconds: 4 } })
assert.equal(invalidReactionDurationResponse.status, 400)
const { data: hiddenReactionRows } = await request(`/rest/v1/session_reaction_prompts?id=eq.${reactionRows.id}&select=id`, { token: invitee.token })
assert.deepEqual(hiddenReactionRows, [])
const { response: forgedReactionResponse } = await request('/rest/v1/rpc/respond_reaction_prompt', { token: invitee.token, method: 'POST', body: { prompt_id: reactionRows.id, should_accept: true } })
assert.equal(forgedReactionResponse.status, 403)
const { data: acceptedReaction, response: acceptedReactionResponse } = await request('/rest/v1/rpc/respond_reaction_prompt', { token: owner.token, method: 'POST', body: { prompt_id: reactionRows.id, should_accept: true } })
assert.equal(acceptedReactionResponse.status, 200)
assert.equal(acceptedReaction.status, 'accepted')
const { data: reactionEvents } = await request(`/rest/v1/session_events?metadata->>reaction_prompt_id=eq.${reactionRows.id}&select=title,body`, { token: owner.token })
...
assert.equal(transferOwnershipResponse.status, 404)

const { response: requireApprovalResponse } = await request(`/rest/v1/campaigns?id=eq.${campaignId}`, { token: owner.token, method: 'PATCH', body: { requires_join_approval: true } })
...
const { response: banApplicantResponse } = await request(`/rest/v1/campaign_members?campaign_id=eq.${campaignId}&user_id=eq.${applicant.id}`, { token: owner.token, method: 'PATCH', body: { status: 'banned' } })
